/**
 * to esella グラインディングファンデーション（nine/lp/grfoundation/base_cp クーポンLP）通常版
 *
 * 録画ベース: recordings/recording.mjs （codegen録画 → 整備）
 *
 * pack / handserum との大きな差分:
 *  - CTAは「グラインディングファンデーションを購入してみる」
 *  - 商品選択ステップあり（.image_box → .product_image_box の2段階画像クリック）
 *  - ボタン名が「次へ」ではなく画面ごとに固有:
 *      「連絡先の入力にすすむ」「お届け先の入力にすすむ」「支払い方法の選択にすすむ」
 *      「配送種類の選択に進む」「今すぐご注文完了する」
 *  - 連絡先画面に電話＋メール＋マイページパスワードが同画面（packはパスワードがカード後だった）
 *  - 住所画面は郵便番号＋番地（建物名フィールドは録画に出現せず → optional）
 *  - 支払い方法選択ラベルが「クレジット（手数料0円）」（packは「クレジットカード」）
 *  - カード入力後に「配送種類の選択」ステップ → 「協力する」クリック → 最終
 *  - サンキュー文言: 「ご購入ありがとうございました」（pack/handserumは「最後にコース内容...」）
 */

const TARGET_URL = 'https://www.toesella.com/nine/lp/grfoundation/base_cp.html';
...
export async function run({ page, customer, recorder }) {
  ...
  await recorder.phase('チャット起動 (CTAクリック)', async () => {
    await page
      .getByRole('link', { name: 'グラインディングファンデーションを購入してみる' })
      .first()
      .click();
