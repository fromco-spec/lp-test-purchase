/**
 * to esella クリスタブライトセラム（cbserum/base_cp クーポンLP）通常版
 *
 * 録画ベース: recordings/recording.mjs （codegen録画 → 整備）
 *
 * pack/handserumに近い構造（「次へ」ベース）だが固有の差分:
 *  - CTAは「まずは試してみる」
 *  - チャット起動直後にオファー確認画面で「次へ」1回押す
 *  - 商品選択ステップ無し（handserum通常版と違う）
 *  - マイページパスワードは住所入力の前（packはカード後）
 *  - 住所は郵便番号＋番地のみ（建物名フィールド無し → optional）
 *  - 支払い方法は「クレジットカード（手数料0円）」(packと同じ)
 *  - 配送種類・性別・生年月日などの追加情報ステップ無し（ファンデと違う）
 *  - 最終ボタン「ご注文完了へ」、サンキュー文言は pack/handserum と同じ想定
 */

const TARGET_URL = 'https://www.toesella.com/cbserum/base_cp.html';
...
export async function run({ page, customer, recorder }) {
  ...
  await recorder.phase('チャット起動 (CTAクリック)', async () => {
    await page.getByRole('link', { name: 'まずは試してみる' }).first().click();
