/**
 * to esella ホワイトハンドセラム（base_soku_cp / 即チャ版）
 *
 * 通常版 (toesella-handserum.mjs) との差分:
...
const TARGET_URL = 'https://www.toesella.com/biyoueki/lp/hand/base_soku_cp.html';
...
export async function run({ page, customer, recorder }) {
  ...
  await recorder.phase('電話番号・メールアドレス入力', async () => {
  ...
  await recorder.phase('注文確認画面へ', async () => {
  ...
  await recorder.phase('完了確認 (サンキュー文言検出)', async () => {
    // 通常版と同じ文言を一旦そのまま採用。実機確認後に必要なら追加する。
