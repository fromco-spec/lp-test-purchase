/**
 * to esella 炭酸パック（nine/lp/pack/base_soku_cp / 即チャ版）
 *
 * 通常版 (toesella-pack.mjs) との差分:
 *   - URLが base_soku_cp.html
 *   - LP着地時点でBOTが自動起動するため、CTAクリックフェーズを削除
 *
 * その他の構造（フォーム順序・全角変換・追加情報・サンキュー文言）は通常版と同じ前提。
 */

const TARGET_URL = 'https://www.toesella.com/nine/lp/pack/base_soku_cp.html';
